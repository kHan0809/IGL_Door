...
for pickle_data in os.listdir(os.getcwd()+'/data_IGL'):
    if 'Inter_mid_sg0' in pickle_data:
...
for i in range(1,len(data_concat)-1):
    for j in range(i+1,len(data_concat)):
        ...
        for coef in coefs:
            ...
            num1 = 1
            x, y, z = zip(*np.array(robot_candi1[-num1:, :3]))
            ax.scatter(x, y, z, color='m', alpha=1.0)
            r = R.from_quat(np.array(robot_candi1)[-num1:,3:7])
            # ==z==
            ...
            x, y, z = zip(*np.array(robot_candi2[-num1:, :3]))
            ax.scatter(x, y, z, color='m', alpha=1.0)
            r = R.from_quat(np.array(robot_candi2)[-num1:,3:7])
            # ==z==
            U, V, W = zip(*(r.as_matrix()[:, :, 2] / 200))
            ax.quiver(x, y, z, U, V, W, color='b')
            # ==y==
            ...
            ax.scatter(x,y,z,color='c',alpha=0.5)
