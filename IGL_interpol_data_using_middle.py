...
import math
import quaternion
from Common.Interpolate_traj import traj_interpolation_Door
...
for i in range(len(data_concat)-1):
    for j in range(i+middle_point,len(data_concat)):
        ...
        if i == 0 and j == middle_point:
            for coef in coefs:
                fixed_traj = traj_interpolation_Door(robot_candi1,robot_candi2,obj_candi1,obj_candi2,sub_goal1[0],coef) # 여기 sub goal은 계속 바뀌어야 된다~~0 나중에 바꿔주셈
