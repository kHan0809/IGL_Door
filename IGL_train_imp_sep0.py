...
for x,batch in enumerate(grid_batch):
    ...
    for y,lr in enumerate(grid_lr):
        for z, wd in enumerate(grid_wd):
            ...
            agent.load_state_dict(torch.load('./model_save/SEP_IGL_sg0_imp002'))
            agent.to(device)
            ...
            torch.save(agent.state_dict(), './model_save/FineSEP_IGL_sg'+subgoal+'_imp'+str(x)+str(y)+str(z))